...
kickBallCoefficient = 0.5;                                                       % A constant used in the KickBall function, which determines the strength of the kick towards the goal. Renamed as "kickBallCoefficient" in KickBall function.
...
if distanceToBall < actionBallDistance
    ...
    if  whatTodo <= kickLikeRange || distanceToGoal < actionGoalDistance  % If the random number falls within the range of kicking or the player is close to the goal
        ...
        ball = BallKicking(ball, kickBallSigma, kickBallCoefficient, targetPosition, timeDelta);  % Kick the ball
