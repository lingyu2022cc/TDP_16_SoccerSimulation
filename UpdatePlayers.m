...
for indexOfPlayer = 1:nPlayers % iterate over each player
    [updatedPlayer, updatedBall] = UpdatePlayer(players, ball, indexOfPlayer, timeDelta, playerOriginalPosition); % update the state of the current player
...
updatedBall = UpdateBallPosition(ball, timeDelta, acceleration); % update the position of the ball based on its current state and acceleration

[updatedPlayers{1}(:,1),updatedPlayers{1}(:,2),updatedBall] = Collisions(updatedPlayers{1}(:,1),updatedPlayers{1}(:,2),updatedBall,particleRadius); % update the positions of the players and ball after collisions
end
